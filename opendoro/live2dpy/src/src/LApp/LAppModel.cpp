...
LAppModel::LAppModel() : CubismUserModel(), _modelSetting(NULL), _userTimeSeconds(0.0f) {
    ...
    __textureManager = std::make_shared<LAppTextureManager>();
}
...
void LAppModel::PreloadMotionGroup(const csmChar* group) {
    ...
    for (csmInt32 i = 0; i < count; i++) {
        ...
        CubismMotion* tmpMotion =
            static_cast<CubismMotion*>(LoadMotion(buffer, size, name.GetRawString()));
...
void LAppModel::Update() {
    ...
    if (_expressionManager != NULL) {
        _expressionManager->UpdateMotion(_model,
                                         deltaTimeSeconds);  // 表情でパラメータ更新（相対変化）
    }
...
CubismMotionQueueEntryHandle LAppModel::StartMotion(
    ...
    if (motion == NULL) {
        ...
        buffer = CreateBuffer(path.GetRawString(), &size);
        motion =
            static_cast<CubismMotion*>(LoadMotion(buffer, size, NULL, onFinishedMotionHandler));
...
CubismMotionQueueEntryHandle LAppModel::StartMotion(
    ...
    CubismMotion* motion = static_cast<CubismMotion*>(_motions[id.c_str()]);

    motion->SetFinishedMotionHandler(onFinishedMotionHandler);

    if (!sound_file.empty()) {
        _wavFileHandler.Start(sound_file.c_str());
    }
...
void LAppModel::SetupTextures() {
    for (csmInt32 modelTextureNumber = 0; modelTextureNumber < _modelSetting->GetTextureCount();
        ...
        LAppTextureManager::TextureInfo* texture =
            __textureManager->CreateTextureFromPngFile(texturePath.GetRawString());
