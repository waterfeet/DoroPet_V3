...
public:
    ...
    * @brief 描画する。
    */
    void Render(int width, int height, CubismMatrix44 *_viewMatrix, LAppModel* model, LAppView *view);

    /**
    * @brief 画像の初期化を行う。
    */
    void InitializeSprite(int width, int height);
    ...
    * @param[in]       pointY            スクリーンY座標
    */
    void OnTouchesMoved(float pointX, float pointY, LAppModel* model) const;
    ...
    * @param[in]       pointY            スクリーンY座標
    */
    void OnTouchesEnded(float pointX, float pointY, LAppModel* model) const;
